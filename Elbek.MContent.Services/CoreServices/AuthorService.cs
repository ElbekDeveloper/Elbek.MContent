...
namespace Elbek.MContent.Services.CoreServices {
  public interface IAuthorService {
    ...
    Task<MContentResult<AuthorDto>> UpdateAuthorAsync(Guid id,
                                                      AuthorDto authorDto);
  ...
  public class AuthorService : IAuthorService {
    ...
    public AuthorService(IAuthorRepository repository, IMapper mapper,
                         IAuthorValidationService validationService) {
      _repository = repository;
      _mapper = mapper;
      _validationService = validationService;
    }
    /// TODO 5 общее замечание по названию методов
    /// это AuthorService и в нем есть метод AddAuthorAsync, Author в названии
    /// метода уже не нужен нужно переименовать в AddAsync или CreateAsync и так
    /// везде
    public async Task<MContentResult<AuthorDto>>
    AddAuthorAsync(AuthorDto authorDto) {
      // validate
      /// TODO 4 authorWithSimilarId и authorWithSimilarName не испотльзуются в
      /// этом методе эти две переменные используются только в валидации нужно
      /// перенести вызовы репозиториев в валидационный сервис тоже самое в
      /// методе на Update
      var authorWithSimilarId = await _repository.GetByIdAsync(authorDto.Id);
      var authorWithSimilarName =
          await _repository.GetAuthorByName(authorDto.Name);

      var validationResult = _validationService.ValidateAddAuthor(
          authorDto, authorWithSimilarId, authorWithSimilarName);
    ...
    public async Task<MContentResult<AuthorDto>>
    UpdateAuthorAsync(Guid id, AuthorDto authorDto) {
      var authorWithSimilarId = await _repository.GetByIdAsync(authorDto.Id);
      var authorWithSimilarName =
          await _repository.GetAuthorByName(authorDto.Name);

      var validationResult = _validationService.ValidateUpdateAuthor(
          id, authorDto, authorWithSimilarId, authorWithSimilarName);
      if (!validationResult.IsValid) {
        return validationResult.ConvertFromValidationResult<AuthorDto>();
      }
      // map to domain object
