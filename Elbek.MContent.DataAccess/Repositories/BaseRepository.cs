...
namespace Elbek.MContent.DataAccess.Repositories {
  public abstract class BaseRepository<TModel>
      : IRepository<TModel> where TModel : BaseEntity {
    protected readonly MContentContext _dbContext;

    public BaseRepository(MContentContext dbContext) { _dbContext = dbContext; }

    /// TODO 6 GenerateQuery - этот метод не генерирует запрос, он какбы
    /// предоставляет доступ к данным и служит для построения запросов. Лучше
    /// назвать его просто Query
    ...
    public virtual async Task<TModel> GetByIdAsync(Guid id) {
      return await GenerateQuery().AsNoTracking().SingleOrDefaultAsync(
          m => m.Id == id);
    }
